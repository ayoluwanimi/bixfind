...
import { Wallet as WalletIcon, ArrowUpRight, ArrowDownLeft, Plus, Minus, X, Check, CreditCard, History } from 'lucide-react'
...
export default function WalletPage() {
  ...
  const handleAddFunds = () => {
    ...
    const updated = {
      ...
      transactions: [
        { id: `tx_${Date.now()}`, type: 'credit', amount: val, description: 'Funds added', date: new Date().toISOString() },
        ...wallet.transactions,
      ],
    }
    setWallet(updated)
    ...
    <div className="min-h-screen bg-gray-950 text-white" suppressHydrationWarning>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <p className="text-white/60 mt-1">Manage your balance and transactions</p>
        </motion.div>

        {/* Balance Card */}
        <motion.div
          className="mt-6 bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 text-white rounded-2xl p-8 relative overflow-hidden"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
          {[
            { label: 'Total Credits', value: `\u20A6${totalCredits.toLocaleString()}`, color: 'text-green-400', icon: ArrowUpRight },
            { label: 'Total Debits', value: `\u20A6${totalDebits.toLocaleString()}`, color: 'text-red-400', icon: ArrowDownLeft },
            { label: 'Transactions', value: wallet.transactions.length.toString(), color: 'text-blue-400', icon: History },
            { label: 'Active Balance', value: `\u20A6${wallet.balance.toLocaleString()}`, color: 'text-purple-400', icon: CreditCard },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        {showWithdraw && (
          ...
          >
            ...
            >
              ...
              <p className="text-xs text-white/30 mb-4">Withdrawals processed within 24 hours.</p>
              <button
