...
export default function CustomerLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentUser: AvatarDropdownUser = {
    id: user.id || '1',
    name: user.user_metadata?.full_name || user.fullName || user.name || 'User',
    email: user.email || '',
    role: 'customer',
    avatar: user.avatar,
  }
  ...
  return (
    <AppShell
      user={currentUser}
      searchSlot={<SearchBar onSearch={handleSearch} size="sm" placeholder="Search services, providers..." />}
      showSearch
      showSidebar
      sidebarProps={{
        role: 'customer',
