...
// Simple sanitization (no external deps)
...
// Advanced sanitization (DOMPurify-compatible without dependency)
export const sanitizeHtml = (html: string): string => {
  // Strip all tags except allowed ones
  const allowed = /<(\/?(b|i|em|strong|p|br|ul|ol|li|a)[^>]*)>/gi
  const safe = html.replace(/<(?!\/?(b|i|em|strong|p|br|ul|ol|li|a)\b)[^>]+>/gi, '')
    .replace(/<(a[^>]*)href=["']javascript:[^"']*["']/gi, '<$1href="#"')
  return safe || escapeHtml(html)
}
...
// Audit log storage with real-time event support
const AUDIT_STORAGE_KEY = 'bixfind_audit_logs'
const AUDIT_EVENT_NAME = 'bixfind:audit'
...
export const auditLogger = {
  ...
  log: (action: string, userId: string, userName: string, details: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem(AUDIT_STORAGE_KEY, JSON.stringify(auditLogger.logs))
      window.dispatchEvent(new CustomEvent(AUDIT_EVENT_NAME, { detail: logEntry }))
    }
  ...
  getLogs: (): AuditLog[] => {
    if (typeof window !== 'undefined') {
      ...
      if (stored) {
        try { auditLogger.logs = JSON.parse(stored) } catch {}
      }
    }
  ...
  clearLogs: () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      window.dispatchEvent(new CustomEvent(AUDIT_EVENT_NAME, { detail: null }))
    }
  },

  subscribe: (callback: (log: AuditLog | null) => void) => {
    ...
    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener(AUDIT_EVENT_NAME, handler)
      }
    }
  }
}